/*
*****************************************************************************
Curso: Tecnologia em Análise e Desenvolvimento de Sistemas
Disciplina: Linguagem e Técnicas de Programação
Professor: Flores
Turma: ADS2A
Componentes:
 26013478-2 - Davi Ferreira da Costa
 26008617-2 - Fernando Cesar Cravo
 26008668-2 - Gabriel Eduardo Consorte
 26010149-2 - Jean Carlos
 26007982-2 - João Gabriel Subtil
Data: 19 de Setembro de 2026
Descritivo: Este programa lê o raio de uma circunferência e calcula seu
comprimento utilizando a constante PI com valor 3.1415 e a fórmula C = 2 * PI * Raio.
*****************************************************************************
*/

<?php

define("PI", 3.1415);

$raio = floatval(readline("Digite o raio: "));

$comprimento = 2 * PI * $raio;

echo "Comprimento da circunferência: " . $comprimento;

?>